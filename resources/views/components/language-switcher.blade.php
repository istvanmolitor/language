        ...
        <div class="absolute right-0 mt-2 w-44 rounded-md border border-slate-200 bg-white shadow-lg z-20">
            <ul class="py-1" role="listbox">
                @foreach($enabledLanguages as $language)
                    <li>
                        <a href="{{ route('language.switch', ['code' => $language->code]) }}"
                           class="flex items-center gap-2 px-3 py-1.5 text-sm hover:bg-slate-50 {{ $currentLanguage->id === $language->id ? 'text-slate-900 font-medium' : 'text-slate-700' }}">
                            <span class="uppercase">{{ $language->code }}</span>
                            <span class="truncate">{{ $language->name ?? strtoupper($language->code) }}</span>
                        </a>
                    </li>
                @endforeach
            </ul>
